...
function handleFloatingButtonDragStart(_position: { x: number; y: number }, event: PointerEvent) {
...
function handleFloatingButtonDragMove(position: { x: number; y: number }, event: PointerEvent) {
  if (dragStartPoint) {
...
<template>
  <button
    ...
    ref="buttonRef"
    type="button"
    class="bg-background/95 hover:bg-muted focus-visible:ring-ring fixed z-60 inline-flex h-11 touch-none items-center gap-2 rounded-full border px-4 text-sm font-medium shadow-lg backdrop-blur transition select-none focus-visible:ring-2 focus-visible:outline-none"
  ...
  >
    <span class="h-2.5 w-2.5 rounded-full" :class="open ? 'bg-emerald-500' : 'bg-primary'" />
    RPC
  </button>
  ...
  <Dialog :open="open" @update:open="open = $event">
    ...
    >
      <DialogHeader class="flex flex-none flex-row items-center justify-between gap-3 space-y-0">
        <div class="min-w-0">
          <DialogTitle class="text-sm">RPC 调试面板</DialogTitle>
          <DialogDescription class="text-xs"> 点击遮罩或按 Esc 关闭 </DialogDescription>
        </div>
        <Button
          variant="destructive"
          class="hover:bg-muted inline-flex h-8 items-center rounded-md border px-3 text-sm transition"
          @click="open = false"
        >
          关闭
        </Button>
