...
export const postProcesses: Record<string, ((...arg: any[]) => Promise<any>)[]> = {
  "0.3.7": [enableDnsTask],
};

export async function applyPostProces(agentUUID: string, oldVersion: string, newVersion: string) {
  const breakPoints = Array.from(Object.keys(postProcesses))
    .sort(compareVersions)
    .filter(
      (bpVer) => compareVersions(bpVer, oldVersion) > 0 && compareVersions(bpVer, newVersion) <= 0,
    );
...
async function enableDnsTask(agentUUID: string) {
  ...
  const exist = !!config.currentUpstream.allow_task_type.find((v) => v === "dns");
  if (!exist) {
