...
<template>
  <main class="bg-background text-foreground grid min-h-dvh place-items-center p-4">
    <Card class="border-border/70 w-full max-w-xl overflow-hidden shadow-sm">
      <CardHeader class="space-y-3 p-5 pb-3">
        <div class="flex items-center justify-between gap-3">
          ...
          <div class="bg-muted/40 grid size-8 place-items-center rounded-full border">
            <ShieldQuestionIcon class="text-muted-foreground size-4" />
          </div>
        </div>

        <div class="space-y-1">
          <CardTitle class="text-2xl font-semibold tracking-tight"> 页面走丢了 </CardTitle>
      ...
      <CardContent class="space-y-4 p-5 pt-2">
        <section class="bg-muted/30 rounded-lg border">
          <div class="flex items-center justify-between gap-3 border-b px-3 py-2">
            <p class="text-foreground text-xs font-medium">路由信息</p>

            <Badge variant="outline" class="h-5 rounded-full px-2 text-[10px]"> debug </Badge>
          </div>
          ...
          <dl class="max-h-72 divide-y overflow-auto">
            ...
            >
              ...
              <dd>
                <pre
                  class="text-foreground font-mono text-xs leading-5 break-all whitespace-pre-wrap"
                  >{{ item.value }}</pre
                >
              </dd>
            </div>
